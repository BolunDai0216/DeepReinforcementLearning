...
def store_data(data, datasets_dir="./data"):
    ...
    data_file = os.path.join(datasets_dir, 'data-%s.pkl.gzip' %
...
def save_results(episode_rewards, results_dir="./results"):
    ...
    fname = os.path.join(results_dir, "results_manually-%s.json" %
                         datetime.now().strftime("%Y%m%d-%H%M%S"))
    fh = open(fname, "w")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--collect_data", action="store_true",
                        default=False, help="Collect the data in a pickle file.")
